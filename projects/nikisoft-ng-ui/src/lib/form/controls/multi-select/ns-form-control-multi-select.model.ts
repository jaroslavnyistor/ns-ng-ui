...
import { LocalizationLanguagesService, NsObject } from 'nikisoft-utils';
...
export abstract class NsFormControlMultiSelectModel<
  TEntity,
...
> extends NsFormControlModel<TEntity, NsFormGroup, NsFormControlMultiSelectConfiguration<TService, TMultiSelectItem>> {
  ...
  performFullTextSearch($event: KeyboardEvent) {
    ...
    this._searchTimeoutId = setTimeout(() => this.loadData(search), 400);
  }
  ...
  private loadData(search: string) {
    ...
    this.subscribeTo(this._service.getLoadListObservable(search), {
