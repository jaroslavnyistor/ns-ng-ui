...
@Injectable()
export class NsUserLogInInformationModel extends NsServiceProviderComponentModel<
...
> {
  ...
  private getIsLoginButtonVisible$(router: Router): Observable<boolean> {
    return router.events.pipe(
      ...
      flatMap((isInLoginScreen) =>
        this.authService.isLoggedIn$.pipe(map((isLoggedIn) => !isLoggedIn && !isInLoginScreen)),
      ),
    );
  }

  private getFullName$() {
    return this.authService.changes$.pipe(flatMap((auth) => of(auth.fullName)));
  }

  private getAdditionalLines$() {
    return this.authService.changes$.pipe(flatMap((auth) => of([auth.userName, auth.email])));
  }
}
